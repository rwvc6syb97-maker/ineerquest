/**
 * P02 人格类型总览（/personality-types，公开）
 * -------------------------------------------------------------
 * 16 型按 NT/NF/SJ/SP 四大族群分组网格展示 + 族群筛选。
 * 卡片点击跳 P03 详情页（/personality-types/:typeCode）。
 * 复用静态数据（personalityTypes）与共享组件（TypeAvatar/GroupBadge/Card/Reveal）。
 * 设计落点：族群色氛围、Reveal 滚动揭示（内置 reduced-motion 友好）。
 */
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button
                ...
                style={active ? { backgroundColor: accent ?? '#3b82f6' } : undefined}
              >
                {f.label}
              </button>
            );
